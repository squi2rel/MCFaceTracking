...
public class EyeTrackingRect extends TrackingRect {
    ...
    public void validate(boolean init) {
        if (init) {
            ...
            lid.validate(true);
            inner.validate(true);
            ball.w = MathHelper.clamp(ball.w, 0, 8 - ball.x);
            ball.h = MathHelper.clamp(ball.h, 0, 8 - ball.y);
            lid.w = MathHelper.clamp(lid.w, 0, 8 - lid.x);
            lid.h = MathHelper.clamp(lid.h, 0, 8 - lid.y);
            inner.w = MathHelper.clamp(inner.w, 0, 8 - inner.x);
            inner.h = MathHelper.clamp(inner.h, 0, 8 - inner.y);
        } else {
            percent = MathHelper.clamp(percent, 0, 1);
            rawPos.x = MathHelper.clamp(rawPos.x, -2, 2);
            rawPos.y = MathHelper.clamp(rawPos.y, -2, 2);
        }
    }
